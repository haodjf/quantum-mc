...
saveresults = 1;
strslit3 = sprintf('%.1f', d3*1e3);
if saveresults
    strnow = "a3_v290_"+strslit3+ ...
        "-" + string(datetime('now','TimeZone','local','Format','y-M-d_HH-mm-ss_z'));
    mkdir(strnow)
...
spec_i=[0.15 0.3 0.45 0.6 0.75 0.9 0.15 0.3 0.45 0.6 0.75 0.9]; %功率
c=zeros(det_num*length(spec_i),1);
l=0.0000; %slit3中心位置
tic; %开始计时
for l_det=1:1:length(spec_i)      %选择功率
    ...
    elapsed = toc;%已用时间
    remaining = (elapsed/l_det)*(length(spec_i)-l_det);%估算剩余时间
    fprintf('进度：%d/%d |已用：%.2f | 剩余： %.2fs\n',l_det, length(spec_i),elapsed,remaining)
end
output = reshape(c,length(spec_x),length(spec_i))';
output = [spec_i' output];
[unique_power, ~, idx] = unique(output(:,1));
merged_data = splitapply(@mean, output(:,2:end), idx)';
merged_power = unique_power;

if saveresults
    save(strnow+ "\workspace" + ".mat",'output','merged_data','merged_power')
end